---
name: webnovel-write
description: Writes webnovel chapters in Vietnamese style (default 2000-2500 words). Use when the user asks to write a chapter or runs /webnovel-write. Runs context, drafting, review, polish, and data extraction. Built-in Contract follows Vietnamese webnovel patterns per STYLE_GUIDE_VN.md.
...
- `/webnovel-write --fast`：Step 1 → 2A → 3 → 4 → 5 → 6 (skip 2B)
- `/webnovel-write --minimal`：Step 1 → 2A → 3 (only 3 basic reviews) → 4 → 5 → 6
...
- Normalize variables:
  ...
  - `SKILL_ROOT`: Skill directory (fixed `${CLAUDE_PLUGIN_ROOT}/skills/webnovel-write`)
