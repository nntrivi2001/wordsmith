---
name: webnovel-plan
description: Builds volume and chapter outlines for Vietnamese webnovels from the total outline, inherits creative constraints, and prepares writing-ready chapter plans. Use when the user asks for outlining or runs /webnovel-plan. Strand/Cool-point methodology follows STYLE_GUIDE_VN.md patterns.
---
...
if [ -z "${CLAUDE_PLUGIN_ROOT}" ] || [ ! -d "${CLAUDE_PLUGIN_ROOT}/skills/webnovel-plan" ]; then
  echo "ERROR: CLAUDE_PLUGIN_ROOT not set or directory missing: ${CLAUDE_PLUGIN_ROOT}/skills/webnovel-plan" >&2
  exit 1
fi
