...
|---|---|---|---|
| `/wordsmith-init` | `wordsmith-init` Skill | New project creation, deep initialization phase | `scripts/init_project.py` + generate `idea_bank.json` |
| `/wordsmith-plan` | `wordsmith-plan` Skill | When volume/chapter outline generation completes and state is written back | `scripts/update_state.py --volume-planned ...` |
| `/wordsmith-write` | `wordsmith-write` Skill | Step 5 data chain update during the writing workflow | Task calls `data-agent` (which internally writes state/index) |
| `/wordsmith-query` | `wordsmith-query` Skill | When analysis requests for "foreshadowing urgency / Strand pacing" are made | `scripts/status_reporter.py --focus urgency/strand` |
| `/wordsmith-resume` | `wordsmith-resume` Skill | Interruption detection, cleanup, and breakpoint recovery | `scripts/workflow_manager.py detect/cleanup/clear` |
...
| `${CLAUDE_PLUGIN_ROOT}/scripts/init_project.py` | `wordsmith-init` Skill | Project initialization phase | Responsible for project scaffolding and base state files |
