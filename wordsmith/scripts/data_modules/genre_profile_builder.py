...
def build_composite_genre_hints(genres: List[str], refs: List[str]) -> List[str]:
    ...
    hints: List[str] = []
    hints.append(
        f"Use \"{primary}\" as the main engine to drive the main plot, with at least 1 \"{'/'.join(secondaries)}\" characteristic expression per chapter."
    )
    if refs:
        hints.append(f"Composite genre execution reference: {refs[0]}")
    hints.append("When primary and secondary genres conflict, prioritize ensuring the primary genre's reader promise; use secondary genre to create freshness.")
    return hints
