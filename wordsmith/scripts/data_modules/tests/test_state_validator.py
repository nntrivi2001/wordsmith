...
def test_to_positive_int_and_resolve_chapter_field():
    ...
    item = {"added_chapter": "Ch15", "target": "200"}
...
def test_status_and_tier_normalization():
    ...
    assert is_resolved_foreshadowing_status("Already recovered") is True
...
def test_normalize_foreshadowing_item_and_chapter_meta_entry():
    ...
    assert normalized_item["content"] == "Ruins key"
    ...
    state = {
        "chapter_meta": {
            "0003": {"coolpoint_pattern": "Counterkill, Mask removal"},
            "7": {"patterns": ["Setback", "Counterkill"]},
        }
    }
    meta3 = get_chapter_meta_entry(state, 3)
    assert meta3["coolpoint_patterns"] == ["Counterkill", "Mask removal"]

    meta7 = get_chapter_meta_entry(state, 7)
    assert meta7["coolpoint_patterns"] == ["Setback", "Counterkill"]
...
def test_normalize_state_runtime_sections():
    state = {
        "plot_threads": {
            "foreshadowing": [
                {"content": "Foreshadowing A", "status": "active", "tier": "decor", "chapter": 11, "target": 99},
                "invalid",
            ]
        },
        "chapter_meta": {
            1: {"cool_point_pattern": "Authority slap|Setback"},
            "bad": "invalid",
        },
    }
    ...
    assert chapter_meta["1"]["coolpoint_patterns"] == ["Authority slap", "Setback"]
