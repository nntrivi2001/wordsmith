...
class ContextManager:
    ...
    SUMMARY_SECTION_RE = re.compile(r"##\s*Summary\s*\r?\n(.*?)(?=\r?\n##|\Z)", re.DOTALL)
    ...
    def _is_snapshot_compatible(self, cached: Dict[str, Any], template: str) -> bool:
        """Determine if snapshot can be used for current template."""
        if not isinstance(cached, dict):
            return False

        meta = cached.get("meta")
        if not isinstance(meta, dict):
            # Backward compatibility: if template not recorded, only allow default template reuse
    ...
    def _build_pack(self, chapter: int) -> Dict[str, Any]:
        ...
        global_ctx = {
            ...
            "power_system_skeleton": self._load_setting("power_system"),
    ...
    def _compute_writing_checklist_score(
        ...
        for item in checklist:
            ...
            if completed:
                ...
                pending_labels.append(str(item.get("label") or item.get("id") or "unnamed item"))
    ...
    def _load_setting(self, keyword: str) -> str:
        ...
        return f"[{keyword}setting not found]"
...
def main():
    ...
    try:
        ...
        print_error("CONTEXT_BUILD_FAILED", str(exc), suggestion="Please check project structure and dependency files")
        try:
