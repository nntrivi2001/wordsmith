...
class IndexReadingMixin:
    ...
    def get_recent_reading_power(self, limit: int = 10) -> List[Dict]:
        """Get recent chapters' reading power metadata"""
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
    ...
    def get_pattern_usage_stats(self, last_n_chapters: int = 20) -> Dict[str, int]:
        """Get cool point pattern usage statistics for the last N chapters"""
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
    ...
    def get_hook_type_stats(self, last_n_chapters: int = 20) -> Dict[str, int]:
        """Get hook type usage statistics for the last N chapters"""
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
    ...
    def get_writing_checklist_score(self, chapter: int) -> Optional[Dict[str, Any]]:
        """Get writing checklist score for a specific chapter."""
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_recent_writing_checklist_scores(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent chapter writing checklist scores."""
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
    ...
    def get_writing_checklist_score_trend(self, last_n: int = 10) -> Dict[str, Any]:
        """Get writing checklist score trend statistics."""
