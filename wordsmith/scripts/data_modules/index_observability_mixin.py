...
class IndexObservabilityMixin:
    ...
    def resolve_invalid_fact(self, invalid_id: int, action: str) -> bool:
        """Confirm or dismiss invalid marking"""
        action = action.lower()
        with self._get_conn() as conn:
            cursor = conn.cursor()
            if action == "confirm":
