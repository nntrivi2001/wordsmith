...
GLOBAL_REGISTRY_REL: Path = Path("wordsmith") / "workspaces.json"
...
def _normcase_path_key(p: Path) -> str:
    """
    Generate stable path key (case-insensitive/path-separator-insensitive on Windows).

    Note: key is only used for mapping table index, actual path still stored as original absolute path string.
    """
    try:
...
def _save_global_registry(path: Path, data: dict) -> None:
    # Write is best-effort: user directory permissions/read-only drives should not block main flow.
    try:
...
def _resolve_project_root_from_global_registry(
    ...
    *,
    ...
    Security policy:
    - Prefer using workspace_hint / CLAUDE_PROJECT_DIR hints for matching.
    - By default, do not use last_used fallback to avoid accidentally targeting wrong project when "completely contextless".
    """
    ...
    # 2) Prefix match (when running from workspace subdirectory)
    for hint in hints:
    ...
    # 3) last_used (optional, default off)
...
def update_global_registry_current_project(
    *,
    ...
    """
    Update user-level registry: workspace -> current_project_root mapping.

    Returns: registry file path (returns None on write failure).
    """
...
def write_current_project_pointer(project_root: Path, *, workspace_root: Optional[Path] = None) -> Optional[Path]:
    ...
    if ws_root is None:
        # Fallback: if `.claude/` cannot be found, treat the project's parent directory as a workspace candidate,
        # used only for writing to the user-level registry (do not create `.claude/` directory or pointer file).
        ws_root = root.parent if root.parent != root else None
    # Note: ws_root may be None (e.g., globally installed skills/agents with no `.claude/` in the workspace).
    # Such cases still need to write to the user-level registry to support subsequent "contextless" resolution.

    pointer_file: Optional[Path] = None
    if ws_root is not None:
        # Only write pointer when `.claude/` already exists in workspace, to avoid creating `.claude/` in arbitrary directories.
    ...
    # Best-effort update user-level registry (non-blocking)
    try:
...
def resolve_project_root(explicit_project_root: Optional[str] = None, *, cwd: Optional[Path] = None) -> Path:
    ...
    if explicit_project_root:
        ...
        # Compatibility: explicitly passed "workspace root" but its `.claude/` is in the user directory (global install),
        # workspace may not have a pointer file inside. In this case, look up from user-level registry.
    ...
    # User-level registry fallback (enabled only when "context hints" are available to avoid false matches)
    # - If CLAUDE_PROJECT_DIR exists: consider that Claude Code provides workspace context
    # - Otherwise, enabled only when base is within a recorded workspace (prefix match)
